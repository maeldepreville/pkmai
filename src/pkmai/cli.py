...
def run_mirror(
    force: bool = typer.Option(False, "--force", "-f", help="Force overwrite of existing mirror notes (ignores cache).")
):
    """
    Generate thesis/antithesis author mirrors for your notes.
    """
    console.print(Panel("[bold cyan]MODULE:[/bold cyan] Author Mirror Generation", border_style="cyan"))
    
    if force:
        console.print("[bold yellow][WARN][/bold yellow] Force mode enabled: Existing mirrors may be overwritten.")

    try:
        author_mirror_notes.main()
        console.print("[bold green][SUCCESS][/bold green] Author Mirror generation complete.")
...
def run_links():
    ...
    """
    console.print(Panel("[bold cyan]MODULE:[/bold cyan] Semantic Auto-Links Generation", border_style="cyan"))
    
    try:
        auto_links.main()
        console.print("[bold green][SUCCESS][/bold green] Auto-Links generation complete.")
    except Exception as e:
        console.print(f"[bold red][ERROR][/bold red] System failure: {e}")
        raise typer.Exit(code=1)
...
def run_server(port: int = typer.Option(8000, help="Port to run the API server on.")):
    ...
    """
    console.print(Panel(f"[bold cyan]MODULE:[/bold cyan] API Server started on port {port}", border_style="cyan"))
