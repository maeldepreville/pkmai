...
def load_cached_embedding(conn: sqlite3.Connection, rel_path: str) -> tuple[str, int, np.ndarray] | None:
...
def delete_missing_embed_entries(conn: sqlite3.Connection, existing_rel_paths: set[str]) -> None:
    ...
    if to_delete:
        conn.executemany("DELETE FROM note_embeddings WHERE rel_path = ?", [(p,) for p in to_delete])
        conn.commit()